...
class www58com(ParserBase):
    ...
    htmlwrite = HtmlFile()
    lou = re.compile(r'([\S]*?)\s*层?[（(]?\s*共\s*(\d*)\s*层\s*[)）]?')
    ...
    def getitem(self, urlbase):
        ...
        try:
            ...
            if 'weizhi' in dir():
                lr[metadatas[7]] = weizhi.find('a', text=re.compile(lr[metadatas[6]].strip()))['href']
            else:
        ...
        try:
            #小区地址
            lr[metadatas[8]] = xq.find_all('p')[1].text.split("：")[1].strip()
        except Exception as e:
            lr[metadatas[8]] = ''
        try:
            #字段地址
            if lr[metadatas[8]] == '':
                addr = soup.find('div', text=re.compile('地址')).find_next('div').contents[0]
                if isinstance(addr, NavigableString):
                    lr[metadatas[8]] = str(addr).strip()
        except:pass
        ...
        try:
            hx = soup.find('div', class_='su_tit', text='户型：')
            #建筑面积
            jzmj = (soup.find('div', class_='su_tit', text='面积：') \
                    or hx).parent
            #print(metadatas[11], re.search(www58com.mi, hx.text).group(1))
            lr[metadatas[15]] = re.search(www58com.mi, jzmj.text).group(1)
            #提供给后边使用
            hx = hx.parent
        except Exception as e:
            lr[metadatas[15]] = ''
        ...
        try:
            ...
            sj = infoPart.find(lambda x:re.compile('售价：').match(x.text) is not None) \
                 or infoPart.find('div', text=re.compile('价格'))
        ...
        try:
            ...
            lr[metadatas[23]] = jg.group(1) if jg else re.search(r'(\d+.?\d*)\s*万', sj.parent.text).group(1).strip()
        except Exception as e:
            lr[metadatas[23]] = ''
        ...
        try:
            ...
            lr[metadatas[33]] = ''
            if fydes.find('p', class_='broker_r_p'):
                lr[metadatas[33]] = fydes.find('p', class_='broker_r_p').text.split('该经纪人')[0].strip()
            elif infoPart.find('div', text=re.compile(r'联系人：')):
                lr[metadatas[33]] = infoPart.find('div', text=re.compile(r'联系人：')).parent.find('a').getText(strip=True)
            elif soup.find('div', class_='su_tit', text=re.compile(r'联系人：')):
                lr[metadatas[33]] = soup.find('div', class_='su_tit', text=re.compile(r'联系人：')).parent.find('a').getText(strip=True)
            elif soup.find('li', class_='jjreninfo_des_jjr'):
                lr[metadatas[33]] = (soup.find('li', class_='jjreninfo_des_jjr').find('a') \
                                     or soup.find('li', class_='jjreninfo_des_jjr').find('span')).getText(strip=True)
        except Exception as e:
            lr[metadatas[33]] = ''
        ...
        try:
            ...
            if fydes.find('p', class_='broker_r_p'):
                ...
                lr[metadatas[34]] = infoPart.find('div', text=re.compile(r'联系人：')).parent.find('a')['href']
            elif soup.find('div', class_='su_tit', text=re.compile(r'联系人：')):
                lr[metadatas[34]] = soup.find('div', class_='su_tit', text=re.compile(r'联系人：')).parent.find('a')['href']
            elif soup.find('li', class_='jjreninfo_des_jjr'):
                lr[metadatas[34]] = soup.find('li', class_='jjreninfo_des_jjr').find('a')['href']
        except Exception as e:
            lr[metadatas[34]] = ''
        ...
        if lr[metadatas[34]] != '' and 'my.58' not in lr[metadatas[34]]:
            ...
            try:
            #经纪公司
                if re.findall('"I":10276,"V":"(.*?)"',reText,re.S):
                    lr[metadatas[35]] = re.findall('"I":10276,"V":"(.*?)"',reText,re.S)[0]
                elif soup.find('div', id='jjrinfo').find(class_='cominfo_name'):
                    lr[metadatas[35]] = soup.find('div', id='jjrinfo').find(class_='cominfo_name').getText(strip=True)
                try:
                    lr[metadatas[36]] = soup.find('div', class_='broker_dl').find('dt', text=re.compile(r'门店：')).find_next('dd').getText(strip=True)
                except:pass
                try:
                    lr[metadatas[38]] = soup.find('div', class_='broker_dl').find('dt', text=re.compile(r'服务区域：')).find_next('dd').getText(strip=True)
                except:pass
            except Exception as e:
                lr[metadatas[35]] = ''
        ...
        try:
            if lr[metadatas[35]] == '':
                if lr[metadatas[34]]:
                    userid = lr[metadatas[34]].split('/')[-2]
                    rjjr = requests.get('http://user.58.com/userdata',
                                     params=dict(userid=userid, type='10', dispcateid='1'),
                                     headers=self.headers, timeout=(3.05, 1.5))
                    lr[metadatas[35]] = re.sub(r'["\']*', '', list(filter(lambda x:'corpname' in x, rjjr.text.split(',')))[0].split(':')[1])
                    if lr[metadatas[33]] == '':
                        lr[metadatas[33]] = re.sub(r'["\']*', '', list(filter(lambda x:'name' in x, rjjr.text.split(',')))[0].split(':')[1])
        except:pass
        ...
        try:
            ...
            lr[metadatas[2]] = pos[0].split('：')[1].strip()
            #如果行政区以路结尾而且片区没有,就放入片区BUG编号9
            if lr[metadatas[2]].endswith("路") and lr[metadatas[3]] == "":
                lr[metadatas[3]] = lr[metadatas[2]]
                lr[metadatas[2]] = ''
        except Exception as e:
            lr[metadatas[2]] = ''
...
if __name__ == '__main__':
    ...
    ###w58.getitem(UrlBean('http://yx.58.com/ershoufang/25853705089855x.shtml', 'www58com#getpages', param='北京',order='12345632156'))
    # w58.getitem(UrlBean('http://ale.58.com/ershoufang/25877455873836x.shtml', 'www58com#getpages', param='北京',order='12345632156'))
    ls = ('http://bj.58.com/ershoufang/26228505415992x.shtml',
