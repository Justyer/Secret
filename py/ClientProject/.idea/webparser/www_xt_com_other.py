...
class wwwxtcomother(ParserBase):
    ...
    def getitem(self, urlbase):
        ...
        try:
            ...
            lr[metadatas[12]] = ''
        try:
            dq_floor = re.findall('<dd>([^共]*?)[/<]',str(floor),re.S)[0]                     #正则  当前层
            lr[metadatas[13]] =  _strip(dq_floor).replace('层','')
        except:
            lr[metadatas[13]] = ''
        try:
...
if __name__ == '__main__':
    ...
    wxt.getitem(UrlBean('http://ls.cityhouse.cn/forsale/0019374514321.html', 'wwwxtcomother#getitem', param=9, headers='哈尔滨',order='123456'))
    wxt.getitem(UrlBean('http://aq.cityhouse.cn/forsale/0002346426811.html', 'wwwxtcomother#getitem', param=9, headers='哈尔滨',order='123456'))
