...
class wwwfangcomSC(ParserBase):
    ...
    def getitem(self, urlbase):
        ...
        try:
            floor = (re.findall('</span>层：</span>(.*?)</dd>',reText,re.S) \
                     or [str(soup.find('span', text=re.compile('楼\s*层：?')).nextSibling)])[0]
        ...
        try:
            ...
            lr[metadatas[24]] = u_price[0].replace('O', '').replace('㎡','')
        except:
            lr[metadatas[24]] =''
        ...
        try:
            for xz_area1 in xz_area:
             ...
             lr[metadatas[2]] =''
        try:
            lr[metadatas[8]] = soup.find('div', class_='info rel floatr').find('span', text=re.compile('地 址：')).parent.getText(strip=True).replace('地 址：', '')
        except:
            lr[metadatas[8]] = ''
...
if __name__ == '__main__':
    ...
    wfang.getitem(UrlBean('http://esf.cangzhou.fang.com/cs/3_153041228.htm', 'wwwfangcomSC#getitem', param='北京',order='1234'))
