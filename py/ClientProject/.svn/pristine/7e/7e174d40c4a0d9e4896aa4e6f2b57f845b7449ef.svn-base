...
class wwwgjcom(ParserBase):
    ...
    def getitem(self, urlbase):
        ...
        try:
            louceng = re.findall('楼<i class="letter-space-8"></i>层：</span>(.*?)</li>',reText,re.S)[0].strip()
        ...
        try:
            ...
            lr[metadatas[13]] = re.search('(.*?)层?[(（/]', louceng).group(1)
        except:
            lr[metadatas[13]] = ''

        try:
            #lr[metadatas[12]] = _strip(louceng.split('/')[1])
            lr[metadatas[12]] = re.search('共(.*)层', louceng).group(1)
        except:
            lr[metadatas[12]] = ''
...
if __name__ == '__main__':
    ...
    ls = ('http://xianyang.ganji.com/fang5/2018408583x.htm',
